    ...
    if response.status_code == 200:
        ...
        if choise:
            ...
            if response.status_code in (200, 201):
                ...
                # Kiểm tra nhiều trường hợp khóa có thể xảy ra (nSSIId hoặc nssiId)
                nssi_id = res_json.get('nSSIId') or res_json.get('nssiId') or res_json.get('templateId')
                if nssi_id:
                    click.echo('Nssi ID: {}'.format(nssi_id))
                else:
                    click.echo('Server Response: {}'.format(res_json))
            else:
                click.echo('OperationFailed: Status Code {}'.format(response.status_code))
                click.echo('Lý do từ Server: {}'.format(response.text))
