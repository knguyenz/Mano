    ...
    def create_vnf_package(self, moi_config):
       ...
       # In ra console để bạn kiểm tra data trước khi gửi
        ...
        if create_vnfp.status_code == 201:
            ...
            # Dòng này cực kỳ quan trọng để xem NFVO chửi gì
            print(f"NFVO Error Detail: {create_vnfp.text}")
            response = {
                "attributeListOut": {
                    'create_vnf_package': create_vnfp.status_code,
                    'nfvo_response': create_vnfp.text # Thêm cái này để NM hiện log lỗi
                },
